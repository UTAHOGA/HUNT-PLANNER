...
function buildDwrHuntUnitPopupContent(boundaryName, matches) {
  ...
  if (singleMatch) {
    return `
      <div style="font-family:Segoe UI,Arial,sans-serif;color:#2b1c12;line-height:1.3;max-width:280px;">
        ${logoHtml}
        <div style="font-size:11px;font-weight:800;letter-spacing:0.04em;text-transform:uppercase;color:#6c43c8;">DWR Hunt Unit</div>
        <div style="font-size:16px;font-weight:800;margin-top:2px;">${escapeHtml(boundaryName)}</div>
        <div style="font-size:12px;margin-top:8px;"><strong>Hunt #:</strong> ${escapeHtml(getHuntCode(singleMatch))}</div>
        <div style="font-size:12px;"><strong>Species:</strong> ${escapeHtml(getSpeciesDisplay(singleMatch))}</div>
        <div style="font-size:12px;"><strong>Sex:</strong> ${escapeHtml(getNormalizedSex(singleMatch))}</div>
        <div style="font-size:12px;"><strong>Hunt Type:</strong> ${escapeHtml(getHuntType(singleMatch))}</div>
        <div style="font-size:12px;"><strong>Weapon:</strong> ${escapeHtml(getWeapon(singleMatch))}</div>
        <div style="font-size:12px;"><strong>Dates:</strong> ${escapeHtml(getDates(singleMatch))}</div>
        <div style="margin-top:10px;">
          <button type="button" onclick="window.selectHuntByCode && window.selectHuntByCode('${escapeHtml(getHuntCode(singleMatch))}')" style="width:100%;border:none;border-radius:999px;padding:8px 12px;font-weight:700;cursor:pointer;background:linear-gradient(180deg,#bf6b34,#9e5323);color:#fff6ed;">Select Hunt</button>
        </div>
      </div>
    `;
  }
